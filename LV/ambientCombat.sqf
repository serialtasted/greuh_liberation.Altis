/*
	...
	Parameters:
		...
		center unit(s) 		= 	unit/array	(unit/object/array which is center of all action) 					DEFAULT: _syncedUnit
		...
		MP					= 	true/false	(true = 'center unit' will automatically be an array of human		DEFAULT: false
								_syncedUnits and everything will be synced around them)
								
		Fully customized example:
				nul = [150,600,10,30,8,[0,1,1],_syncedUnit,[0.2,0.3,0.1,0.55,0.25,1,1,0.25,1,1],1,800,"hint format['spawning unit: %1',this];",["AWARE","SAD"],false] execVM "LV\ambientCombat.sqf";
...
_maxRange = if(count _this > 1)then{_this select 1;} else {700};	 
_minTime = if(count _this > 2)then{_this select 2;} else {420};	 
_maxTime = if(count _this > 3)then{_this select 3;} else {900};	 
_groupAmount = if(count _this > 4)then{_this select 4;} else {3};
_sideRatios = if(count _this > 5)then{_this select 5;} else {[0.5,1,0]}; 
_syncedUnit = if(count _this > 6)then{_this select 6;} else {_syncedUnit};	 
_skills = if(count _this > 7)then{_this select 7;} else {"default"};	 
_communication = if(count _this > 8)then{_this select 8;} else {1};	 
_dissapearDistance = if(count _this > 9)then{_this select 9;} else {1000};	 
_customInit = if(count _this > 10)then{_this select 10;} else {nil};	 
_patrolType = if(count _this > 11)then{_this select 11;} else {["SAFE","SAD"]};	
...
if(_mp)then{if(isNil("LV_Get_syncedUnits"))then{LV_Get_syncedUnits = compile preprocessFile "LV\LV_functions\LV_fnc_get_syncedUnits.sqf";};};
...
if( [] call F_opforCap < (GRLIB_battlegroup_cap + 50) ) then {
	if( (_syncedUnit distance startbase) > 1200 && (_syncedUnit distance (getMarkerPos "respawn_west")) > 200 && !([1400, (getpos _syncedUnit)] call F_getNearestSector in blufor_sectors) ) then {
		
		diag_log format["### SPAWNING AMBIENT COMBAT"];
		
		if(count LV_ACS_activeGroups < _groupAmount)then{
			if(count LV_ACS_activeGroups == (_groupAmount - 1))then{sleep _timeDelay;};
			if(_mp)then{ _syncedUnit = call LV_Get_syncedUnits;};
			...
			if(!(surfaceIsWater _spawnPos)) then{
				_landOrAir = floor(random 5);
				if(_landOrAir > 2)then{
					_grp = [_spawnPos, _side] call LV_fullAirVehicle;
				}else{
					_grp = [_spawnPos, _side, [10,3]] call LV_menGroup;
					_grp2 = [_spawnPos, _side] call LV_fullLandVehicle;
					(units group _grp2) joinSilent _grp;
				};
			} else {
				_grp = [_spawnPos, _side] call LV_fullAirVehicle;
			};
		...
		diag_log format["### AMBIENT COMBAT NOT SPAWNED"];
	};
};
