...
WEST:		nul = [player, 1, true, false, 1400, "random", true, 250, 50, 6, 0, 230, false, false, false, true, player, false, "default", nil, nil, 1,false] execVM "LV\heliParadrop.sqf";
EAST:		nul = [player, 2, true, false, 1400, "random", true, 250, 150, 6, 2, 230, false, false, false, true, player, false, "default", nil, nil, 1,false] execVM "LV\heliParadrop.sqf";
...
_BLUchopper = BLU_ParaHeli;
_OPFchopper = OPF_ParaHeli;
_INDchopper = IND_ParaHeli;
...
_spos = [(_landingSpotPos select 0) + (sin _heliDirection) * _heliDistance, (_landingSpotPos select 1) + (cos _heliDirection) * _heliDistance, 1300];
_heli = createVehicle [_heliType, _spos, [], 0, "FLY"];
createVehicleCrew _heli;
(crew _heli) joinSilent _grp;
{ _x setVariable [ "SAVEUNIT", false ]; } forEach crew _heli;

_heli allowDamage false;

if(_captive)then{
	_heli setCaptive true;
	{ _x setCaptive true; } forEach crew _heli;
};
...
[_heli, _allowDamage] spawn { sleep 5; (_this select 0) allowDamage (_this select 1); };
...
for "_i" from 1 to _jumperAmount step 1 do{
	...
	if(_jumpDelay > 0) then{ sleep _jumpDelay };
};
