...
while { !CSclosed } do {
	...
	while { dialog && ( !CSclosed ) } do {
		...
		_controlText = ((findDisplay 2490) displayCtrl 1015); _controlButton = ((findDisplay 2490) displayCtrl 2401); if ( _unitrank >= 3 ) then { if ( _limitGrenadier <= 0 && _limitGrenadier != -99 ) then { _controlText ctrlSetText "Available kits: 0"; _controlButton ctrlEnable false; } else { if ( _limitGrenadier != -99 ) then { _available = format [ "Available kits: %1", _limitGrenadier ]; _controlText ctrlSetText _available; } else { _controlText ctrlSetText "Available kits: ∞"; }; _controlButton ctrlEnable true; }; } else { _controlText ctrlSetText "Rank too low"; _controlButton ctrlEnable false; };
