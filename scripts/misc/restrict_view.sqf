...
[{
	...
	if ( player distance ( [] call F_getNearestFob ) > GRLIB_fob_range && player distance ( getpos lhd ) > (GRLIB_fob_range + 50) && player distance ( getpos nimitz ) > (GRLIB_fob_range + 100) ) then {
