...
#define JOIN_DEFAULT JOIN_DISABLED		 
...
#define ACCEPT_DEFAULT ACCEPT_DISABLED 
...
GVAR(canTakeLeadership) = {
	...
	if(isNil{aero_playtime_get}) then {
		if([(leader _THIS(0))] call F_getPlayerRank > [_THIS(0)] call F_getPlayerRank) then { 
			_out = false;
		};
	} else {
...
GVAR(menu_main) = {
	...
	if ( _unitTeam isEqualTo "PTr_alpha" || _unitTeam isEqualTo "PTr_bravo" || _unitTeam isEqualTo "PTr_charlie" ) then {
		...
		//---------------------------------------------- SQUADS
		if ( _unitTeam != "PTr_alpha" ) then {
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Join Squad Alpha",
					{ ["PTr_alpha"] call F_setPlayerTeam; },
					[],
					9093
				]
			PUSH_END
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> You are on Squad Alpha",
					{ },
					[],
					9093
				]
		...
		if ( _unitTeam != "PTr_bravo" ) then {
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Join Squad Bravo",
					{ ["PTr_bravo"] call F_setPlayerTeam; },
					[],
					9092
				]
			PUSH_END
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> You are on Squad Bravo",
					{ },
					[],
					9092
				]
		...
		if ( _unitTeam != "PTr_charlie" ) then {
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Join Squad Charlie",
					{ ["PTr_charlie"] call F_setPlayerTeam; },
					[],
					9091
				]
			PUSH_END
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> You are on Squad Charlie",
					{ },
					[],
					9091
				]
...
"Squad manager is active, hold T and use mousewheel" call GVAR(msg);
