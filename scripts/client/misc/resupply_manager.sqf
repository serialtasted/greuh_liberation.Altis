	...
	if ( _veh != player ) then {
		if ( effectiveCommander _veh == player ) then {
			if ( (speed _veh < _repair_speed) && (((getPosATL _veh) select 2) < _repair_altitude) ) then {
				...
				if ( count ( (getpos _veh) nearEntities [ repair_vehicles , _resupply_dist] ) > 0 ) then {
				...
				if ( ( count ( (getpos _veh) nearEntities [ ammo_vehicles , _resupply_dist] ) > 0 ) && ( _rearm_ticker < _rearm_time ) ) then {
					_rearmed = true;
					_rearm_ticker = _rearm_ticker + 1;
					if ( _rearm_ticker >= _rearm_time ) then {
						[_veh] remoteExec ["F_rearmVehicle"];
					};
				};

				if ( count ( (getpos _veh) nearEntities [ fuel_vehicles , _resupply_dist] ) > 0 ) then {
					if ( fuel _veh < ( 1 - _refuel_amount ) )  then {
						_refueled = true;
						[_veh, (fuel _veh + _refuel_amount)] remoteExec ["F_setFuel"];
					};
				};
			} else {
				_rearm_ticker = 0;
