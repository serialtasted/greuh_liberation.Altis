...
while { true } do {
	if (  [ player, 5 ] call F_fetchPermission  ) then {
		...
		_near_people = (getPosATL player) nearEntities [["Man"], 4];
		{
			if ( (captive _x) && !(_x in _actionned_captive_units) && !((side group _x) == WEST) && (_x distance ([] call F_getNearestFob) < (GRLIB_fob_range - 80)) ) then {
