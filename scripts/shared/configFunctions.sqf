...
ISSE_Cfg_ItemInfo = {
    private["_cfg", "_name", "_DescShort", "_DescLong", "_Pic", "_Type"];
    _name = _this;
    _cfg = (configFile >> "CfgItems" >> _name);
    _DescShort = if (isText(_cfg >> "displayName")) then {
        getText(_cfg >> "displayName")
    }
    else {
...
ISSE_Cfg_EquipmentInfo = {
    private["_cfg", "_name", "_DescShort", "_DescLong", "_Pic", "_Type"];
    _name = _this;
    _cfg = (configFile >> "CfgEquipment" >> _name);
    _DescShort = if (isText(_cfg >> "displayName")) then {
        getText(_cfg >> "displayName")
    }
    else {
...
ISSE_Cfg_Items_GetName  = {
    (_this call ISSE_Cfg_ItemInfo) select 0
};

ISSE_Cfg_Items_GetDesc  = {
    (_this call ISSE_Cfg_ItemInfo) select 1
};

ISSE_Cfg_Items_GetType  = {
    (_this call ISSE_Cfg_ItemInfo) select 2
};

ISSE_Cfg_Items_GetPic   = {
    (_this call ISSE_Cfg_ItemInfo) select 3
};

ISSE_Cfg_Equipment_GetName  = {
    (_this call ISSE_Cfg_EquipmentInfo) select 0
};

ISSE_Cfg_Equipment_GetDesc  = {
    (_this call ISSE_Cfg_EquipmentInfo) select 1
};

ISSE_Cfg_Equipment_GetType  = {
    (_this call ISSE_Cfg_EquipmentInfo) select 2
};

ISSE_Cfg_Equipment_GetPic   = {
    (_this call ISSE_Cfg_EquipmentInfo) select 3
};
